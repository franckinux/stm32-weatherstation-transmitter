...
static uint16_t i2c_timeout = 1000;
...
// return:
//   HAL_ERROR if there was a timeout during I2C operations, HAL_OK otherwise
HAL_StatusTypeDef I2Cx_Read(
    uint8_t *bufw, uint32_t nbytesw, uint8_t *bufr, uint32_t nbytesr
) {
    HAL_StatusTypeDef status;

    status = HAL_I2C_Master_Transmit(&hi2c1, BME280_ADDR, bufw, nbytesw, i2c_timeout);
    if (status != HAL_OK) {
      return status;
    }
    status = HAL_I2C_Master_Receive(&hi2c1, BME280_ADDR, bufr, nbytesr, i2c_timeout);
    if (status != HAL_OK) {
      return status;
    }
...
void BME280_WriteReg(uint8_t reg, uint8_t value)
{
    ...
    status = HAL_I2C_Master_Transmit(&hi2c1, BME280_ADDR, buf, 2, i2c_timeout);
}
...
uint8_t BME280_ReadReg(uint8_t reg)
{
    ...
    // Read register value
    status = I2Cx_Read(&reg, 1, &value, 1);
...
BME280_RESULT BME280_Read_Calibration(void)
{
    ...
    if (I2Cx_Read(buf, 1, (uint8_t *)&cal_param, 24) != HAL_OK) {
      return BME280_ERROR;
    }
    ...
    if (I2Cx_Read(buf, 1, buf, 7) != HAL_OK) {
      return BME280_ERROR;
    }
...
BME280_RESULT BME280_Read_UP(int32_t *UP)
{
    ...
    *UP = 0x80000;
    ...
    if (I2Cx_Read(buf, 1, buf, 3) == HAL_OK) {
...
BME280_RESULT BME280_Read_UH(int32_t *UH)
{
    ...
    *UH = 0x8000;
    ...
    if (I2Cx_Read(buf, 1, buf, 2) == HAL_OK) {
...
BME280_RESULT BME280_Read_UTPH(int32_t *UT, int32_t *UP, int32_t *UH)
{
    ...
    *UH = 0x8000;
    ...
    if (I2Cx_Read(buf, 1, buf, 8) == HAL_OK) {
        *UP = (int32_t)((buf[0] << 12) | (buf[1] << 4) | (buf[2] >> 4));
